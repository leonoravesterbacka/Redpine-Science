...
search; in the other it has web search only. Web search is Tavily in both arms
\todo{add reference: Tavily}. Answers are stored, so every judging pass scores the same
text.
...
of TREC nugget evaluation \todo{add reference: TREC nugget evaluation}. Contradiction
rate is the share of claims the answer contradicts, as in K-QA \todo{add reference:
K-QA}. Correctness combines the two. With $n$ required claims, of which the answer states $s$
...
repeats for consistency. Agreement is measured with Krippendorff's alpha \todo{add
reference: Krippendorff's alpha} and with the intraclass correlation of the judge
...
inference \todo{add reference: prediction-powered inference}, which corrects the judge's
