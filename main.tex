...
\lead{Writing the questions and their answer keys.} A language model pipeline
(Claude Opus 5) turns a paper into a question and an answer key. The pipeline searches
...
weaker is stated as an association or a preliminary finding. The gold also names the
population and setting the claims hold for, and one wrong-if clause: the most tempting
over-claim, which an answer must not make. Passages from other papers on the same
question decide whether a finding is corroborated, uncontested or contested, which sets
...
against the standards our reviewers wrote down and repairs or discards it.

The last step tests the gold before an expert sees it. Three answers are generated for
each question: one that read the paper, one that read a different but valid source, and
one with no retrieval. Each is graded against the gold twice. The gold is kept only when
the first answer scores well, the second is not punished for citing a different paper,
and the third does not pass. This removes golds that reward one paper's phrasing and
golds that a model can satisfy from memory.
...
and could comment. Four rounds went out: 40, 20, 100 and 20 questions. Before each round
we wrote down what would count as a problem: a drop in the expert's good-rate against
their own earlier rounds, or the same defect appearing on three or more of twenty
questions. A rewritten question is taken over once a check confirms that the gold still
answers it. A change to a gold is taken over only if the three test answers, regraded
against the new gold, still pass the same test as before. Of 180 reviewed questions, 125
are unchanged, 40 carry the expert's wording, 14 carry an edited gold and one was
dropped.
...
claims and contradicts the rest scores 0.5. Figure~\ref{fig:q2-example} shows one
question with its gold and the judge's verdicts on one answer.
Citations, journal names and identifiers are removed before judging so that the judge
grades what an answer says, not where it says it came from. Scores that compare an answer
...
with the judge's three verdicts plus cannot assess, one answer per screen and the two
arms of a question several screens apart. Every expert labels a random core of 22
...
\bibitem{tavily}
Tavily.
\newblock Tavily: One secure API for real-time web access.
\newblock \url{https://tavily.com}, accessed September 2026.

\bibitem{nuggetrecall}
R.~Pradeep, N.~Thakur, S.~Upadhyay, D.~Campos, N.~Craswell, and J.~Lin.
\newblock The Great Nugget Recall: Automating Fact Extraction and RAG
  Evaluation with Large Language Models.
\newblock In \emph{Proceedings of the 48th International ACM SIGIR Conference
  on Research and Development in Information Retrieval (SIGIR)}, 2025.
  Preprint \emph{arXiv:2504.15068}.

\bibitem{kqa}
I.~Manes, N.~Ronn, D.~Cohen, R.~Ilan~Ber, Z.~Horowitz-Kugler, and
  G.~Stanovsky.
\newblock K-QA: A Real-World Medical Q\&A Benchmark.
\newblock In \emph{Proceedings of the 23rd Workshop on Biomedical Natural
  Language Processing (BioNLP)}, pages 277--294, Bangkok, Thailand, 2024.
...
\newblock Answering the Call for a Standard Reliability Measure for Coding
  Data.
\newblock \emph{Communication Methods and Measures}, 1(1):77--89, 2007.
...
\bibitem{ppiplusplus}
A.~N. Angelopoulos, J.~C. Duchi, and T.~Zrnic.
