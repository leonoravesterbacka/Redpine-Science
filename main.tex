...
\usepackage{amsmath}
\usepackage{charter}              % clean serif body face, no extra font files needed
\usepackage{parskip}              % no first-line indent, spaced paragraphs
\setlength{\parskip}{0.8em}       % make the inter-paragraph gap unambiguous
\raggedbottom
\usepackage{titlesec}
\usepackage{booktabs}
\usepackage{array}
...
% ---- Author-block macros ---------------------------------------------------
% Accepts \author{First}{Last} (call once per author, accumulates with a
% comma), \affiliation{Org}{Country}, and \keywords{...} -- the two-argument
% call style used by the journal template this report is also typeset in, so
% that block pastes in directly without reformatting.
\newcommand{\rpauthorlist}{}
\renewcommand{\author}[2]{%
  \ifdefempty{\rpauthorlist}%
    {\renewcommand{\rpauthorlist}{#1~#2}}%
    {\appto{\rpauthorlist}{, #1~#2}}%
}
\newcommand{\rpaffiliation}{}
\newcommand{\affiliation}[2]{\renewcommand{\rpaffiliation}{#1, #2}}
\newcommand{\rpkeywords}{}
\newcommand{\keywords}[1]{\renewcommand{\rpkeywords}{#1}}
...
\title{\vspace{-2em}Leveraging a four tiered approach for evaluating Redpine Science}
\date{}

\author{Leonora}{Vesterbacka}
\author{Filip}{Dorm}
\affiliation{Redpine}{Sweden}
...
\noindent\textbf{Keywords:} \rpkeywords

\section{Introduction}

Redpine Science gives models and agents a single access point to peer-reviewed
literature from seven publishers, queried directly through MCP and API. This report
measures how much that access improves the answers a model produces, and how reliably
...
\section{Quadrant one: public benchmarks, answer quality}
\label{sec:quadrant-one}
...
We have six running so far: MedHELM, MMLU-Redux Biology, BioASQ factoid, BioASQ yes/no,
SciFact fact verification, and MedXpertQA. HealthBench, LiveMedBench, and Humanity's
Last Exam's Bio/Chem Gold subset are still being added.
...
\section{Quadrant two: expert-validated questions, answer quality}
\label{sec:quadrant-two}
...
arm is better. Two further criteria score retrieval directly, separate from the answer
score, so a weak final answer built on strong retrieval is distinguishable from one
built on weak retrieval:
...
\captionof{figure}{Preliminary, n=57.}%
}

\section{Quadrant three: public benchmarks, retrieval quality}
\label{sec:quadrant-three}
...
The Exa publication-retrieval benchmark grounds every query in one gold publication and
scores whether it was surfaced, by DOI or title, at what rank. It asks several
...
\section{Quadrant four: expert-validated questions, retrieval quality}
\label{sec:quadrant-four}
...
Judges score each result on a 0 to 4 scale, 0 not relevant, 4 perfectly relevant, blind
to which system produced it; system identity is randomized left-to-right per query to
prevent a judge from guessing rather than reading. Every result is stripped of anything
that would give the system away before a judge reads it: URL, DOI, and publication date
are all removed, since a database record and a scraped web page format these
differently enough to identify the source on their own.
...
Quadrant three depends directly on Exa's decision to publish its publication-retrieval
benchmark openly rather than keep it proprietary. Quadrant one depends similarly on
MMLU-Redux, MedXpertQA, HealthBench, LiveMedBench, MedHELM, BioASQ, and Humanity's Last
Exam, each built and released publicly by its creators; every comparison in this report
against other labs' results is possible because of that choice. Evaluation
infrastructure of this kind is a shared resource, not a competitive moat.

\section{What we are releasing}
