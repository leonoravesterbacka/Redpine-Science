...
\lead{Method.} The rise of public benchmarks has enabled systematic evaluation of LLM capabilities across an increasing range of domains, including coding \todo{add citations}. The biomedical domain is no exception, with a growing number of established benchmarks. However, many widely used public benchmarks are likely to have been incorporated into the training data of the models being evaluated. As a result, performance on some benchmarks may increasingly reflect memorization rather than the ability to retrieve, integrate, and reason over relevant information. Newer and more challenging benchmarks are therefore continually introduced to remain informative as model capabilities improve. On the more difficult benchmarks, retrieval may still provide a measurable benefit, while performance can be independently compared against results reported by other groups on the same tasks.

We evaluate models across a broad set of biomedical benchmarks. These benchmarks span a range of difficulty and susceptibility to memorization. Some are already approaching saturation, with current models answering a large proportion of questions correctly without external retrieval, as reflected in \todo{point to benchmarks}. Others remain sufficiently challenging that access to additional biomedical information may improve performance. This allows us to measure the value of Redpine retrieval across both settings: whether it provides gains where models are already strong, and whether those gains are larger on benchmarks where relevant information remains difficult to access from model parameters alone.
...
text, not its freely available abstract. \todo{add more details on what models and such we use}
...
\lead{Experimental setup.} Two near identical set ups, only differeing by the tools they have access to, run on identical prompts: an agent with access to
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\textwidth]{example-image-duck}
    \caption{Graph: Precision@5, percent of results scored 3 or 4, by system}
    \label{fig:duck_placeholder}
\end{figure}
