...
Sonnet 5 writes and grades the test answers described below. The first 39 questions
came from the same stages run by hand in a model session; every later question came
from the pipeline as code. The pipeline searches
...
valid source, and one with no retrieval. Each is graded against the gold twice and the
scores travel with the question. A gold that the paper-informed answer scores below 0.75
on, or that the retrieval-free answer scores 0.5 or above on, is flagged for the expert:
the first rewards one paper's phrasing, the second can be satisfied from memory. The same
three answers later guard every edit to a gold.
...
against the new gold, keep their scores: the paper-informed answer stays at or above
0.85 and within 0.05 of before, the other-source answer falls by at most 0.05, the
retrieval-free answer rises by at most 0.05, and no new contradiction appears. When either check fails, the
