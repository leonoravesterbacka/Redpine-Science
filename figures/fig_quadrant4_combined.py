...
Data provenance -- same pooled judging pass for both panels: two PhD-level
domain experts each blind-score their own batch of 30 subspecialty questions
(one batch mostly neurology, the other entirely cardiology), top 5 results
per system, 0-4 relevance scale, single-shot retrieval (no agent loop),
"relevant" = score >= 3 of 4, matching quadrant four's Method section. Read
directly from connect-eval-fresh's run outputs:
...
Precision@5: relevant counts derived from each run's precision * n_scored
(every one lands on a whole number). Pooled: Redpine 176/299 = 58.9%,
PubMed 97/298 = 32.6%. Error bars are a Wald 95% CI for a binomial
proportion, from these exact pooled counts.

Mean DCG@5: per-query DCG@5 values from both judges' 30-query batches
concatenated into one list of 60 per system (Jin's 30 followed by Zihan's
30, in query order within each batch) -- the same values fig_quadrant4_dcg.py
carried before its removal, copied forward here rather than re-derived, since
they were already checked against the pooled aggregate ("7.85"/"4.83" to two
decimal places) at the time. Error bars are a two-sided t 95% CI of the mean
over the 60 pooled per-query values (59 df).
...
# --- Panel 1: Precision@5 ---------------------------------------------------
PRECISION_COUNTS = {  # (relevant, n_scored), pooled across both judges
    "redpine": (176, 299),
    "pubmed": (97, 298),
}
...
# --- Panel 2: mean DCG@5 -----------------------------------------------------
# Per-query DCG@5, pooled across both judges' 30-query batches (60 values per
# system): relevance_judging_jin_30's 30 values followed by
...
T_CRIT_59DF = 2.0010  # two-sided t, 95%, 59 df
